  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-1">
        ...
        <Badge variant={stage.kind === 'Interna' ? 'default' : 'secondary'}>
          {stage.kind}
        </Badge>
        {stage.responsible && (
          <span className="text-sm text-muted-foreground">• {stage.responsible}</span>
        )}
      </div>
...
export default function OPPlanner() {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = op.stages.findIndex((s) => s.id === active.id);
      const newIndex = op.stages.findIndex((s) => s.id === over.id);
      reorderStages(op.id, oldIndex, newIndex);
    }
  };
  ...
  const handleAddStage = (name?: string) => {
    ...
    addStage(op.id, stageName);
    setNewStageName('');
    toast.success('Etapa adicionada!');
  };

  const handleRemoveStage = (stageId: string) => {
    removeStage(op.id, stageId);
  ...
  const handleSaveStageConfig = () => {
    ...
    updateStage(op.id, editingStage.id, editingStage);
    setSheetOpen(false);
  ...
  const handleAddChecklist = () => {
    ...
    addChecklistItem(op.id, editingStage.id, newChecklistItem);
    setNewChecklistItem('');
          ...
          <div className="flex items-start justify-between">
            <div>
              <CardTitle className="text-2xl">
                {op.id} {op.publicada && <Badge className="ml-2">Publicada</Badge>}
              </CardTitle>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="sm:max-w-xl overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>{editingStage?.name}</SheetDescription>
          </SheetHeader>
          ...
          {editingStage && (
            <div className="grid gap-6 py-6">
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(value: 'Interna' | 'Terceirizada') =>
                    setEditingStage({ ...editingStage, kind: value })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="grid gap-2">
                ...
                <div className="flex gap-2">
                  <Select
                    value={editingStage.responsible}
                    onValueChange={(value) =>
                      setEditingStage({ ...editingStage, responsible: value })
                    }
                  >
                    <SelectTrigger>
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && newResponsavel.trim()) {
                        ...
                        setEditingStage({ ...editingStage, responsible: newResponsavel });
                        setNewResponsavel('');
                      }
                    }}
                  />
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  min="0"
                  step="0.01"
                  value={editingStage.unitValue || ''}
                  onChange={(e) =>
                    setEditingStage({
                      ...editingStage,
                      unitValue: parseFloat(e.target.value) || undefined,
                    })
                  }
                />
              </div>

              {editingStage.kind === 'Terceirizada' && (
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        min="0"
                        step="0.01"
                        value={editingStage.freightGo || ''}
                        onChange={(e) =>
                          setEditingStage({
                            ...editingStage,
                            freightGo: parseFloat(e.target.value) || undefined,
                          })
                        }
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        min="0"
                        step="0.01"
                        value={editingStage.freightBack || ''}
                        onChange={(e) =>
                          setEditingStage({
                            ...editingStage,
                            freightBack: parseFloat(e.target.value) || undefined,
                          })
                        }
                      />
                    </div>
              ...
              <div className="grid gap-2">
                <Label>Imprevisto (R$)</Label>
                <Input
                  type="number"
                  min="0"
                  step="0.01"
                  value={editingStage.unexpected || ''}
                  onChange={(e) =>
                    setEditingStage({
                      ...editingStage,
                      unexpected: parseFloat(e.target.value) || undefined,
                    })
                  }
                />
              </div>
              ...
              <div className="grid gap-2">
                <Label>Observação do Imprevisto</Label>
                <Textarea
                  value={editingStage.unexpectedNote || ''}
                  onChange={(e) =>
                    setEditingStage({ ...editingStage, unexpectedNote: e.target.value })
                  }
                  rows={2}
                />
              </div>
              ...
              <div className="grid gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {editingStage.checklist.filter((i) => i.done).length}/
                    {editingStage.checklist.length}
                  </Badge>
                </div>
                  ...
                  {editingStage.checklist.map((item) => (
                    ...
                    >
                      <Checkbox
                        checked={item.done}
                        onCheckedChange={() => {
                          toggleChecklist(op.id, editingStage.id, item.id);
                          setEditingStage({
                            ...editingStage,
                        ...
                        <Input
                          ...
                          onBlur={() => {
                            if (editingChecklistText.trim()) {
                              renameChecklistItem(op.id, editingStage.id, item.id, editingChecklistText);
                              setEditingStage({
                                ...editingStage,
                                checklist: editingStage.checklist.map((i) =>
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          removeChecklistItem(op.id, editingStage.id, item.id);
                          setEditingStage({
                            ...editingStage,
              ...
              <div className="p-4 bg-primary/5 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-primary">
                    R$ {getStageTotal(editingStage).toFixed(2)}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground mt-2">
                  {editingStage.unitValue && (
                    <div>Custo unitário: R$ {editingStage.unitValue} × {getTotalPlanejado()} pçs</div>
                  )}
                  {(editingStage.freightGo || editingStage.freightBack) && (
                    <div>
                      Frete: R$ {((editingStage.freightGo || 0) + (editingStage.freightBack || 0)).toFixed(2)}
                    </div>
                  )}
                  {editingStage.unexpected && (
                    <div>Imprevisto: R$ {editingStage.unexpected.toFixed(2)}</div>
                  )}
                </div>
              </div>
            </div>
