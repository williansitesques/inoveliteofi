...
export function UserFormDialog({ open, onOpenChange, onSubmitCreate, onSubmitEdit, mode, editingUser }: Props) {
  ...
  const isProtected = editingUser?.id === 'admin-fixed' || editingUser?.id === 'admin-1';
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Falha ao salvar. Verifique os dados e tente novamente.';
        ...
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[80vh]">
          <div className="overflow-y-auto px-6 pb-6 space-y-6">
            ...
            <div>
              ...
              <Input id="email" type="email" readOnly={mode==='edit' && !!isProtected} {...form.register('email')} />
              {mode==='edit' && isProtected && (
                <p className="text-xs text-muted-foreground">E-mail protegido não pode ser alterado.</p>
              )}
